...
export default class ChildReply extends Component {
  ...
  _newReReply = e => {
    ...
    let { userName, parentsReplyId, } = this.props.reply
    if(!userName){
      userName = '지나가는 행인'
    }
  ...
  _makeReReply = async (e) => {
    ...
    this.setState({show_reReplyInput:false})
  }
       ...
       <ul className='child_reply'>
        ...
        <li className="reply_content">
          ...
          <div className="reply_msg_button">
            ...
            ? null
            : <span className='make_rereply btn_wr_reply' onClick={this._handleReReplyInput}>답글달기</span>
            }
          </div>
        </li>
        <div>
